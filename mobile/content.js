...
function insertComparisonTable(shop, comparisonMessage, retryCount = 0, summary = null, savings = 0, startOpen = true) {
    ...
    async function renderWidgetCart() {
        ...
        complete.slice(1).forEach(([name, s]) => {
            ...
            const priceEl = document.createElement('span'); priceEl.style.fontWeight = '600'; priceEl.textContent = `${s.total} kr.`;
            row.appendChild(nameEl); row.appendChild(priceEl);
        ...
        if (partial.length) {
            ...
            widgetCartPanel.appendChild(sep);
            partial.forEach(([name, s]) => {
                ...
                const nameEl = document.createElement('span'); nameEl.textContent = `${name} (${s.count}/${n})`;
                const priceEl = document.createElement('span'); priceEl.textContent = `${s.total} kr.*`;
                row.appendChild(nameEl); row.appendChild(priceEl);
                widgetCartPanel.appendChild(row);
            });
            ...
            widgetCartPanel.appendChild(note);
        }
        ...
        widgetCartPanel.appendChild(divider);
        cart.forEach(item => {
            const row = document.createElement('div');
            row.style.cssText = 'display:flex;align-items:center;padding:4px 0;gap:6px;';
            const nameEl = document.createElement('div');
            nameEl.style.cssText = 'flex-grow:1;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#555;';
            nameEl.textContent = item.name || 'Ukendt produkt';
            const removeBtn = document.createElement('button');
            removeBtn.textContent = '✕';
            removeBtn.style.cssText = 'background:none;border:none;cursor:pointer;color:#ddd;font-size:12px;padding:0;flex-shrink:0;';
...
/* ═══════════════════════════════════════════════════════════
   MOBILE OVERRIDES — alting herunder erstatter desktop-UI
   Function-hoisting sikrer at disse definitioner vinder.
   ═══════════════════════════════════════════════════════════ */

// Forhindrer desktop auto-init
function handleNavigation() {}
function setupMutationObserver() {}
function addDkkPriceDisplay() {}
function toggleMoreShops() {}
function createToggleButton() {}
function generateComparisonTable() { return ''; }
function setupProductInfo() {}

// Loading-placeholder: opret kun den skjulte status-panel som onShopResult-callback bruger
function insertLoadingPlaceholder(shop, activeShops) {
    let panel = document.getElementById('pp-shop-status');
    if (!panel) {
...
// Ingen GTIN / fejl → åbn tom bottom sheet
function insertComparisonTable(shop, message, retryCount, summary, savings, startOpen) {
    _mobileOpenSheet([], null);
    _mobileResetFAB();
}

// Erstat inline widget med mobil bottom sheet
function displayPrice(responses, identifier, identifierType) {
    ...
    const results = responses
        ...
        .map(r => {
            ...
            let priceText;
            if (shop.inertia) {
                priceText = extractInertiaPrice(r.html, shop);
            } else if (shop.dataProps) {
                priceText = extractDataPropsPrice(r.html, shop);
            } else if (shop.shopifySearch) {
                try {
                    const json = JSON.parse(r.html);
                    const products = json && json.resources && json.resources.results && json.resources.results.products;
                    if (!products || products.length === 0) return null;
                    const cents = parseInt(products[0].price, 10);
                    if (isNaN(cents) || cents <= 0) return null;
                    priceText = String(cents / 100);
                ...
                if (shop.scriptExtract.container) {
                    ...
                    src = Array.from(container.querySelectorAll('script')).map(function(s) { return s.textContent; }).join('\n');
                }
                ...
                if (el) {
                    priceText = (el.textContent.trim() || el.getAttribute('content') || '')
                        .replace(/^(from|fra|ab|dès|vanaf)\s+/i, '').trim();
                }
                if (!priceText) {
                    priceText = extractJSONLDPrice(r.html, identifier) || extractNextDataPrice(r.html, shop);
                }
            }
            if (!priceText) return null;
            var parsed = extractPriceAndCurrency(priceText);
            var price = parsed.price;
            var currency = parsed.currency || shop.defaultCurrency || 'EUR';
            if (!price) return null;
            var dkk = currency === 'DKK' ? price : price * RATES.EUR_TO_DKK;
            return { shop: shop.name, dkk: Math.round(dkk), url: r.url };
        })
        .filter(function(r) { return r !== null; })
        .sort(function(a, b) { return a.dkk - b.dkk; });

    _mobileOpenSheet(results, identifier);
    _mobileResetFAB();

    try {
        var info = getCurrentPriceAndCurrency();
        if (info && info.price) {
            var curDkk = info.currency === 'DKK' ? info.price : info.price * RATES.EUR_TO_DKK;
            var cheaper = results.filter(function(r) { return r.dkk < curDkk - 1; }).length;
            browser.runtime.sendMessage({ action: 'setBadge', count: cheaper });
        }
    } catch(e) {}
}
...
function _mobileResetFAB() {
    if (!_fab) return;
    _fabLabel(_fab, '🔍', 'Priser');
    _fab.style.opacity = '1';
    _fab.disabled = false;
}

function _fabLabel(btn, icon, text) {
    btn.textContent = '';
...
function _injectFAB() {
    ...
    Object.assign(_fab.style, {
        position: 'fixed', bottom: '20px', right: '16px', zIndex: '2147483646',
        background: _ORANGE, color: '#fff', border: 'none', borderRadius: '24px',
    ...
    _fabLabel(_fab, '🔍', 'Priser');
    _fab.addEventListener('click', _onFABClick);
    document.body.appendChild(_fab);
}

async function _onFABClick() {
    _fabLabel(_fab, '⏳', 'Henter…');
    _fab.style.opacity = '0.8';
    _fab.disabled = true;
    gtinSearchAttempts = 0;
    cachedGTIN = null;
    processedGTINs.clear();
...
function _mobileEl(tag, styles) {
    var node = document.createElement(tag);
    if (styles) Object.assign(node.style, styles);
    return node;
}

function _mobileMsgBox(text) {
    var p = _mobileEl('p', { color: '#999', fontSize: '14px', textAlign: 'center', padding: '32px 16px', lineHeight: '1.6' });
    p.textContent = text;
    return p;
}
...
function _mobileCloseSheet() {
    ...
    if (_sheet) _sheet.style.transform = 'translateY(100%)';
    setTimeout(function() {
...
function _mobileOpenSheet(results, gtin) {
    if (_overlay) _mobileCloseSheet();
    ...
    _overlay.addEventListener('click', _mobileCloseSheet);

    _sheet = document.createElement('div');
    Object.assign(_sheet.style, {
    ...
    closeBtn.textContent = '✕';
    closeBtn.addEventListener('click', _mobileCloseSheet);
    hdr.appendChild(logo);
    hdr.appendChild(closeBtn);

    var body = _mobileEl('div', { overflowY: 'auto', flex: '1', padding: '16px 16px 32px' });

    if (!gtin) {
        body.appendChild(_mobileMsgBox('Ingen stregkode fundet på denne side.'));
    } else if (results.length === 0) {
        body.appendChild(_mobileMsgBox('Ingen priser fundet. Produktet er muligvis ikke tilgængeligt i de understøttede butikker.'));
    } else {
        var sub = _mobileEl('p', { fontSize: '12px', color: '#aaa', margin: '0 0 14px', lineHeight: '1.4' });
        ...
        results.forEach(function(r, i) {
            ...
            var left = _mobileEl('div', { display: 'flex', flexDirection: 'column', gap: '4px' });
            var name = _mobileEl('div', { fontWeight: '600', fontSize: '15px', color: '#111' });
            name.textContent = r.shop;
            left.appendChild(name);
            if (cheapest) {
                var badge = _mobileEl('span', {
            ...
            var right = _mobileEl('div', { display: 'flex', alignItems: 'center', gap: '12px' });
            var price = _mobileEl('div', {
                fontWeight: '800', fontSize: '20px',
                color: cheapest ? '#16a34a' : '#111', letterSpacing: '-0.03em'
            });
            price.textContent = r.dkk + ' kr.';
            var arrow = _mobileEl('div', { fontSize: '20px', color: '#ccc', flexShrink: '0' });
            arrow.textContent = '→';
...
function _mobileInit() {
    if (!SHOPS.find(function(s) { return location.hostname.includes(s.domain); })) return;
    _injectFAB();
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _mobileInit);
} else {
    _mobileInit();
}
